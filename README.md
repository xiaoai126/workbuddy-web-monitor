...
> 预览：全局汇总卡片（今日积分/预估费用/tokens）、模型分布、每日用量面板（近 30 天，含积分与费用列，点击某天可筛选当日明细）。
...
- **预估费用**：基于可编辑的 `pricing.json` 定价表（元/百万 tokens）按「模型 → token 用量」精确计算，拆分输入 / 缓存命中 / 输出三项；未收录定价的模型不显示费用（全局卡片带 `+` 号表示还有未计价部分）。卡片上的 **!** 悬停可查看所用单价。支持**高峰/空闲双档价**（DeepSeek：北京时间周一至五 9-12/14-18 为高峰，其余半价，scanner 按时段分桶）；自建网关模型不录入定价即不显示
...
全局面板提供：今日请求/输入/输出、活跃会话数（10 分钟内）、24h 会话数、累计汇总、模型分布条形图、**每日用量列表**。

### 按日筛选

- **每日用量面板**：展示近 30 天每天的请求次数、输入/输出 tokens、积分与预估费用（北京时间口径，含周几与"今天"标记），点击某一行即筛选该天
- 筛选后：汇总卡片变为**当日**口径（请求数/积分/费用含输入·缓存·输出拆分/tokens），会话列表只显示当天有活动的会话，卡片数值均为当天用量，详情历史也仅限当天
- 日期选择器与「全部/活跃/24h/今日」快速筛选互斥；再点一次每日行或清空日期即退出按日模式
- API：`GET /api/daily?days=30`（全局按日聚合）、`GET /api/sessions?date=YYYY-MM-DD`（按日会话列表）、`GET /api/sessions/:id/detail?date=YYYY-MM-DD`（按日详情）
