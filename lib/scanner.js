...
function newStat() {
  return {
    ...
    events: [],
    lastModel: null,
    models: {},          // model -> {requests, input, output, cached, reasoning, credit}（按模型分别累计，供费用估算）
...
async function scanFile(filePath) {
  ...
  rl.on('line', (line) => {
    ...
    if (ev.model) {
      ...
      const slot = (stat.models[ev.model] = stat.models[ev.model] || { requests: 0, input: 0, output: 0, cached: 0, reasoning: 0, credit: 0 });
...
function toSummary(stat, speedInfo) {
  ...
  for (let i = stat.events.length - 1; i >= 0; i--) {
    ...
    if (ev.model) {
      const slot = (todayModels[ev.model] = todayModels[ev.model] || { requests: 0, input: 0, output: 0, cached: 0, reasoning: 0, credit: 0 });
      slot.requests += ev.requests;
      slot.input += ev.input;
      slot.output += ev.output;
      slot.cached += ev.cached;
