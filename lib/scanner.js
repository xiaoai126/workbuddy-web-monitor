...
function newStat() {
  return {
    ...
    daily: {},           // 'YYYY-MM-DD'(北京时间) -> {requests,input,output,cached,reasoning,credit,models:{...}}（按日聚合）
    totalRequests: 0,
    totalInput: 0,
    totalOutput: 0,
    totalCached: 0,
...
/** 北京时间日期键 YYYY-MM-DD（与宿主机时区无关，Docker UTC 下也正确） */
function beijingDateKey(ts) {
  return new Date(ts + 8 * 3600 * 1000).toISOString().slice(0, 10);
}

/** 北京时间当天 0 点的 Unix ms */
function beijingMidnight(ts) {
  return Date.parse(beijingDateKey(ts) + 'T00:00:00+08:00');
}
...
/** 合并两个按模型累计的 slot（用于全局按日聚合） */
function mergeSlot(slot, info) {
  slot.requests += info.requests || 0;
  slot.input += info.input || 0;
  slot.output += info.output || 0;
  slot.cached += info.cached || 0;
  slot.reasoning += info.reasoning || 0;
  slot.credit += info.credit || 0;
  if (info.peak) {
    slot.peak = slot.peak || { input: 0, cached: 0, output: 0 };
    slot.peak.input += info.peak.input || 0;
    slot.peak.cached += info.peak.cached || 0;
    slot.peak.output += info.peak.output || 0;
  }
}

function newModelSlot() {
  return { requests: 0, input: 0, output: 0, cached: 0, reasoning: 0, credit: 0 };
}
...
async function scanFile(filePath) {
  ...
  rl.on('line', (line) => {
    ...
    // 按日聚合（北京时间）
    const dk = beijingDateKey(ev.ts);
    const day = stat.daily[dk] = stat.daily[dk] || { requests: 0, input: 0, output: 0, cached: 0, reasoning: 0, credit: 0, models: {} };
    day.requests += ev.requests;
    day.input += ev.input;
    day.output += ev.output;
    day.cached += ev.cached;
    day.reasoning += ev.reasoning;
    day.credit += ev.credit;
    const peak = isPeakHour(ev.ts);
    if (ev.model) {
      stat.lastModel = ev.model;
      accumulateSlot(stat.models[ev.model] = stat.models[ev.model] || newModelSlot(), ev, peak);
      accumulateSlot(day.models[ev.model] = day.models[ev.model] || newModelSlot(), ev, peak);
    }
  });
...
class Scanner {
  ...
  /** 单会话详情（含请求历史；可按日过滤） */
  detail(sessionId, dateKey) {
    for (const [, entry] of this.cache) {
      if (entry.stat.sessionId !== sessionId) continue;
      const stat = entry.stat;
      let base = stat;
      let speed = entry.speed;
      if (dateKey && stat.daily && stat.daily[dateKey]) {
        base = this._dayStat(stat, dateKey);
        speed = computeSpeeds(base);
      }
      const events = dateKey ? base.events : stat.events;
      return {
        ...toSummary(base, speed),
  ...
  /** 构造某一天的会话级 stat 视图（用于按日摘要/详情） */
  _dayStat(stat, dateKey) {
    const bucket = stat.daily[dateKey] || { requests: 0, input: 0, output: 0, cached: 0, reasoning: 0, credit: 0, models: {} };
    const dayEvents = stat.events.filter((e) => beijingDateKey(e.ts) === dateKey);
    return Object.assign({}, stat, {
      events: dayEvents,
      totalRequests: bucket.requests,
      totalInput: bucket.input,
      totalOutput: bucket.output,
      totalCached: bucket.cached,
      totalReasoning: bucket.reasoning,
      totalCredit: bucket.credit,
      models: bucket.models,
      contextTokens: dayEvents.length ? dayEvents[dayEvents.length - 1].input : 0,
      lastModel: dayEvents.length ? dayEvents[dayEvents.length - 1].model : null,
      firstTs: dayEvents.length ? dayEvents[0].ts : stat.firstTs,
      lastTs: dayEvents.length ? dayEvents[dayEvents.length - 1].ts : stat.lastTs,
    });
  }
  ...
  daySummaries(dateKey) {
    ...
    for (const [, entry] of this.cache) {
      const stat = entry.stat;
      if (!stat.daily || !stat.daily[dateKey]) continue;
      const dayStat = this._dayStat(stat, dateKey);
      out.push(toSummary(dayStat, computeSpeeds(dayStat)));
    }
  ...
  dailyGlobal(days = 60) {
    const map = new Map();
    for (const [, entry] of this.cache) {
      for (const [dk, bucket] of Object.entries(entry.stat.daily || {})) {
        const g = map.get(dk) || { date: dk, requests: 0, input: 0, output: 0, cached: 0, reasoning: 0, credit: 0, models: {} };
        g.requests += bucket.requests;
        g.input += bucket.input;
        g.output += bucket.output;
        g.cached += bucket.cached;
        g.reasoning += bucket.reasoning;
        g.credit += bucket.credit;
        for (const [m, info] of Object.entries(bucket.models || {})) {
          mergeSlot(g.models[m] = g.models[m] || newModelSlot(), info);
        }
        map.set(dk, g);
      }
    }
    return [...map.values()].sort((a, b) => b.date.localeCompare(a.date)).slice(0, days);
  }
}

module.exports = { Scanner, computeSpeeds, beijingMidnight };
